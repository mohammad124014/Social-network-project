...
import deleted from "../../service/delete";
...
const Panel = () => {
  ...
  const [listUsersFollowing, setListUsersFollowing] = useState([]);
  const [listUsersFollower, setListUsersFollower] = useState([]);
  ...
  // const { isLoading: loadGetListFollow, refetch: refetchListFollow } = useQuery(
  //   "getListFollow",
  //   () => get.getListFollowersAndFollowings(headerModalFollow, userId),
  //   {
  //     onSuccess: (response) => {
  //       setListUsersFollow(response?.data);
  //       console.log("yyyyyyyyyyyyyyyyyy");
  //     },
  //     onError: () => {
  //       console.log("ooooooooooooooooooo");
  //     },
  //     enabled: !!headerModalFollow,
  //   }
  // );

  const { isLoading: loadGetListFollowing, refetch: refetchListFollowing } =
    useQuery("getListFollowing", () => get.getListFollowings(userId, userId), {
      onSuccess: (response) => {
        setListUsersFollowing(response?.data);
      },
      onError: () => {
      },
      enabled: !!openModalFollowing,
    });

  const { isLoading: loadGetListFollower, refetch: refetchListFollower } =
    useQuery("getListFollower", () => get.getListFollowers(userId, userId), {
      onSuccess: (response) => {
        setListUsersFollower(response?.data);
      },
      onError: () => {
      },
      enabled: !!openModalFollower,
    });
  ...
  const removedPost = useMutation(
    (postId) => deleted.removePost(userId, postId),
    {
      onSuccess: (response) => {
        setOpenModal(false);
        refetchgetUser();
  ...
  const removePost = () => {
    removedPost.mutate(postSelected?._id);
  };

  // const showFollow = (name) => {
  //   setHeaderModalFollow(name);
  //   refetchListFollow();
  ...
  return (
    <Box component={"section"}>
                ...
                >
                  ...
                  >
                    ...
                    <KeyboardArrowRightIcon
                      sx={{ fontSize: "28px", marginTop: "3.5px" }}
                    />
                  </p>
                </div>
              )}
              ...
              <div>
                <div>
                  ...
                  >
                    ...
                    {userInfo.location ? (
                      <p
                        style={{ fontSize: "16px", color: "rgb(170,170,170)" }}
                      >
                        <i
                ...
                <div>
                  <div style={{ display: "flex", position: "relative" }}>
                    <div
                      onClick={() => {
                        const command = { userId: userId, idUser: userId };
                    ...
                    <MenuTooltip
                      style={
                        openTooltip ? { display: "block" } : { display: "none" }
                      }
                    >
      ...
      {/* <ModalFollow
        ...
        isLoading={loadGetListFollow}
        showUser={false}
        name={headerModalFollow}
        users={listUsersFollow}
        handleOpenModal={setOpenModalFollow}
      /> */}

      <ModalFollow
        openModal={openModalFollowing}
        isLoading={loadGetListFollowing}
        showUser={false}
        name={"following"}
        users={listUsersFollowing}
        handleOpenModal={setOpenModalFollowing}
        refetchFollow={refetchListFollowing}
      />

      <ModalFollow
        openModal={openModalFollower}
        isLoading={loadGetListFollower}
        showUser={false}
        name={"follower"}
        users={listUsersFollower}
        handleOpenModal={setOpenModalFollower}
        refetchFollow={refetchListFollower}
      />
